...
public class MaxXirGenerator implements RiXirGenerator {
    ...
    private void engraveAllocatorId(XirOperand cell, XirOperand etla) {
        if (MaxineVM.useNUMAProfiler) {
            ...
            asm.pload(WordUtil.archKind(), threadId, etla, asm.i(VmThreadLocal.THREAD_INVENTORY_KEY.offset), true);
            asm.mov(allocatorShift, asm.i(LightweightLockword.ALLOCATORID_SHIFT));

            asm.shl(threadId, threadId, allocatorShift);

            asm.pstore(WordUtil.archKind(), cell, asm.i(miscOffset()), threadId, false);
        }
    }
